	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (login.trim().equals("user")) {
			try {
				...
				// storing current user in session using SessionManager
				...
				if (user != null) {
					...
					session.setAttribute("activeUser", user); // For backward compatibility
			...
			try {
				...
				HttpSession session = request.getSession();
				if (admin != null) {
					// Create secure admin session with IP tracking
					String clientIP = SessionManager.getClientIP(request);
					SessionManager.createAdminSession(session, admin, clientIP);
					session.setAttribute("activeAdmin", admin); // For backward compatibility
